...
import { setBalancesByDates, setPlansMatrix, setProgressByCategories } from '../services/calculatings'
  ...
  return [
    watch(
      ...
      async (nv) => {
        ...
        // console.log(nv)
        const setAllCalcs = () => {
          setBalancesByDates({ categories: nv, plans: plans?.value, config: config?.value })
          setProgressByCategories({ categories: nv, plans: plans?.value, actions: actions?.value })
        }
    ...
    watch(
      ...
      async (nv) => {
        ...
        const setAllCalcs = () => {
          ...
          setBalancesByDates({ categories: categories?.value, plans: nv, config: config?.value })
          setProgressByCategories({ categories: categories?.value, plans: nv, actions: actions?.value })
    ...
    watch(
      ...
      async (nv) => {
        ...
        return new Promise((resolve, reject) => {
          try {
            resolve(setProgressByCategories({ categories: categories?.value, plans: plans?.value, actions: nv }))
          } catch (err) {
            reject(err)
          }
        })
    ...
    watch(
      ...
      async (nv) => {
        ...
        return new Promise((resolve, reject) => {
          try {
            resolve(setBalancesByDates({ categories: categories?.value, plans: plans?.value, config: nv }))
          } catch (err) {
            reject(err)
          }
        })
