...
function setPlansMatrix({ plans = [] }) {
  ...
  plans.forEach(({ date, _id, category_id }, idx) => {
    ...
    if (!plansIdsByDatesByCategoriesIds?.[formattedDate]) plansIdsByDatesByCategoriesIds[formattedDate] = {}
...
function setProgressByCategories({ plans = [], actions = [], categories = [] }) {
  ...
  const plansIdsByCategoriesIds = store.getters.getCalcs('plansIdsByDatesByCategoriesIds')?.[thisMonth] || {}
  const plansIndexesByIds = store.getters.getCalcs('plansIndexesByIds')
  const sumsByCategoriesStatuses = store.getters.getCalcs('sumsByDatesByCategoriesStatuses')?.[thisMonth] || {}
  ...
  categories.forEach(({ _id, status }) => {
    ...
    if (sum !== undefined)
      progressByCategoriesIds[_id] = {
        sum,
        percentage: planSum !== undefined ? Math.round((sum / planSum || 0) * 10000) / 100 : undefined,
      }
  })
