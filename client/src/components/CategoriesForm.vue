<template>
  <div class="form-container">
    ...
    <el-form :model="newCategory" :rules="categoryRules" label-position="top" ref="categoryForm">
      <el-form-item label="Название" prop="name">
        <el-input v-model="newCategory.name"></el-input>
      </el-form-item>
      <div>
        <el-form-item label="Направление финансов" prop="status">
          <el-radio-group v-model="newCategory.status">
            <el-radio label="income">Доходы</el-radio>
            <el-radio label="expense">Расходы</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="Тип категории" prop="type">
          <template #label>
            Тип категории
            <InfoBalloon :data="[
    ...
    <div class="form-buttons">
      <el-button v-if="isEditMode" @click="processCategory('delete')" type="danger" :icon="iconDelete" round>
        Удалить
      </el-button>
      <el-button @click="cancelProcessing" :icon="iconCancel" round>
        Отменить
      </el-button>
      <el-button v-if="isEditMode" type="success" @click="processCategory('change')" :icon="iconCheck" round>
        Сохранить
      </el-button>
      <el-button v-else type="primary" @click="processCategory('add')" :icon="iconCheck" round>
        Сохранить
      </el-button>
    </div>

    <el-dialog v-model="redefiningDialog" width="500px" :append-to-body="true" :before-close="handleCancelRedefining">
      <template #header>
        <h4>Переназначить категорию "{{ newCategory.name }}"</h4>
      </template>
      <div class="redefining-container">
        <el-form :model="newCategory" :rules="redefiningRules" label-position="top" ref="redefiningForm">
          <el-form-item label="Категория для переназначения">
            <el-select v-model="newCategory.redefined_category_id" filterable>
        ...
        <div class="form-buttons">
          <el-button @click="handleCancelRedefining" :icon="iconCancel" round>
            Отменить
          </el-button>
          <el-button type="primary" @click="redefineAndDelete" :icon="iconCheck" round>
            Сохранить
          </el-button>
        </div>
      </div>
  ...
  data() {
    return {
      ...
      categoryRules: {
        ...
        status: [{ required: true, message: 'Направление финансов - обязательное поле', trigger: 'blur' }],
        type: [{ required: true, message: 'Тип категории - обязательное поле', trigger: 'blur' }],
      },
      isEditMode: false,
      redefiningDialog: false,
      redefiningRules: {
        redefined_category_id: [{ required: true, message: 'Нужно выбрать категорию для переназначения', trigger: 'blur' }],
      },
    }
  },
  computed: {
    ...
    async processCategory(mode) {
      const process = async () => {
        try {
          ...
          let changes
          if (mode === 'delete') changes = categories.delete(this.newCategory._id, this.newCategory)
      ...
      if (mode === 'delete') {
        const showDeletingConfirm = () => this.$confirm('Восстановить категорию будет нельзя', 'Удалить категорию?', {
        ...
        const isNotEmptyCategory = !![...this.$store.getters.getData('plans'), ...this.$store.getters.getData('actions')]
          .find(({ category_id }) => category_id === this.newCategory._id)

        if (isNotEmptyCategory) return this.$confirm(`Переназначить категорию для планов и операций удаляемой категории?`,
