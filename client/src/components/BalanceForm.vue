  ...
  <div class="form-container">
    <el-form :model="newCheck" :rules="checkRules" label-position="top" ref="checkForm">
      <el-form-item label="Общий баланс" prop="checked_balance">
        ...
        <el-input-number v-model="newCheck.checked_balance" :min="0" :step="100" />
      </el-form-item>
      <el-form-item label="Размер накоплений" prop="checked_savings">
        <el-input-number v-model="newCheck.checked_savings" :min="0" :step="100" />
      </el-form-item>
      <el-form-item label="Дата сверки" prop="checking_date">
        <template #label>Дата сверки
          <InfoBalloon data="Поменяйте, если указываете баланс не на текущий момент." />
        </template>
        <el-date-picker v-model="newCheck.checking_date" :disabled-date="(time) => time.getTime() > Date.now()"
          type="date" placeholder="Выберите дату" format="DD.MM.YYYY" />
      </el-form-item>
    </el-form>
    <div class="form-buttons">
      <el-button type="primary" @click="checkConfig" :icon="iconCheck" round>
        Сохранить
      </el-button>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      newCheck: {},
      checkRules: {
        checked_balance: [{ required: true, message: 'Баланс - обязательное поле', trigger: 'blur' }],
