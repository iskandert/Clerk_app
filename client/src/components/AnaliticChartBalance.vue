...
export default {
  ...
  computed: {
    balances() {
      ...
      const balancesByDatesArrayed = Object.entries(balancesByDates).map(([date, balanceData]) => {
        return {
          date,
        ...
        datasets: [
          ...
          },
          {
            ...
            tooltip: {
              callbacks: {
                label: (context) => {
                  return ' ' + getFormattedCount(this.balances[context.dataIndex].savings || 0, { accuracy: 0 }) + ' - накопления'
    ...
    options() {
      return {
        ...
        scales: {
          y: {
            min: Math.min(0, ...this.balances.map(({ balance }) => balance)) && undefined,
