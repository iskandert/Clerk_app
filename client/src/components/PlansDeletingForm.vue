<template>
  <div class="form-container">
    <h5>Удалить планы</h5>
    <el-form :model="deletingPlan" :rules="planRules" label-position="top" ref="planForm">
      <el-form-item label="Категории" prop="categories_ids">
        <el-select v-model="deletingPlan.categories_ids" filterable default-first-option multiple>
      ...
      <el-form-item label="Начальный месяц" prop="date">
        <el-date-picker v-model="deletingPlan.date" :disabled-date="(time) => $dayjs(time).isBefore($dayjs(), 'month')"
          type="month" placeholder="Выберите дату" format="MMMM YYYY" />
      </el-form-item>
      <el-form-item label="Конечный месяц" prop="dateLast">
        <el-date-picker v-model="deletingPlan.dateLast"
          :disabled-date="(time) => !$dayjs(time).isAfter($dayjs(deletingPlan.date), 'month')" type="month"
    ...
    <div class="form-buttons">
      ...
      <el-button @click="processPlan('delete')" type="danger" :icon="iconDelete" round>
        Удалить
      </el-button>
    </div>
  </div>
  ...
  methods: {
    processPlan() {
      this.$refs.planForm.validate(async valid => {
        ...
        try {
          await this.$confirm('Восстановить планы будет нельзя', 'Удалить выбранные планы?', {
  ...
  watch: {
    'deletingPlan.date': {
      handler(nv) {
        ...
        if (!nv || !this.$dayjs(nv).isBefore(this.$dayjs(this.deletingPlan.dateLast), 'month')) {
