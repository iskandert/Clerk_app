<template>
  <div class="form-container">
    ...
    <el-form :model="extendingSettings" :rules="extendingRules" label-position="top" ref="extendingForm">
      <el-form-item label="Продляемый месяц" prop="date">
        <el-date-picker v-model="extendingSettings.date"
          :disabled-date="(time) => $dayjs(time).isBefore($dayjs(), 'month')" type="month" placeholder="Выберите дату"
          format="MMMM YYYY" />
      </el-form-item>
      <el-form-item label="Конечный месяц" prop="dateLast">
        <el-date-picker v-model="extendingSettings.dateLast"
          :disabled-date="(time) => !$dayjs(time).isAfter($dayjs(extendingSettings.date), 'month')" type="month"
          placeholder="Выберите дату" format="MMMM YYYY" />
      </el-form-item>
      <el-form-item label="Категории" prop="categories_ids">
        <el-select v-model="extendingSettings.categories_ids" filterable default-first-option multiple
          v-if="!isAllCategs">
        ...
        <el-checkbox v-model="isAllCategs" label="Все категории"></el-checkbox>
      </el-form-item>
    </el-form>
    <div class="form-buttons">
      ...
      <el-button @click="processPlan" type="primary" :icon="iconCheck" round>
        Продлить
      </el-button>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      isAllCategs: false,
      extendingRules: {
        categories_ids: [{ required: true, message: 'Нужно выбрать хотя бы одну категорию', trigger: 'blur' }],
        date: [{ required: true, message: 'Нужно выбрать продляемый месяц', trigger: 'change' }],
  ...
  watch: {
    'extendingSettings.date': {
      handler(nv) {
        ...
        if (!nv || !this.$dayjs(nv).isBefore(this.$dayjs(this.extendingSettings.dateLast), 'month')) {
    ...
    isAllCategs(nv) {
      if (nv) this.extendingSettings.categories_ids = this.categoriesStored.map(({ _id }) => _id)
