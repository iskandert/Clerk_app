<template>
  <div class="form-container">
    ...
    <el-form :model="newPlan" :rules="planRules" label-position="top" ref="planForm">
      <el-form-item label="Планируемая сумма" prop="sum">
      ...
      <el-form-item label="Конечный месяц" prop="dateLast">
        <el-date-picker v-model="newPlan.dateLast"
          :disabled-date="(time) => !$dayjs(time).isAfter($dayjs(newPlan.date), 'month')" type="month"
          placeholder="Выберите дату" format="MMMM YYYY" />
      </el-form-item>
      <el-form-item class="comment" label="Комментарий">
        <el-input v-model="newPlan.comment" :rows="2" type="textarea" placeholder="Подробности операции"></el-input>
      </el-form-item>
    </el-form>
